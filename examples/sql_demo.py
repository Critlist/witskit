...
async def demo_comprehensive_sql():
    ...
    try:
        ...
        for source_info in sources_data:
            ...
            for i, frame_data in enumerate(source_info["frames"]):
                ...
                frame = decode_frame(frame_data, source=source, use_metric_units=False)
        ...
        if depth_data:
            depths = [
                dp.parsed_value for dp in depth_data if dp.parsed_value is not None
            ]
        ...
        async for dp in sql_writer.query_data_points(
            symbol_codes=["0108", "0113", "0114"], start_time=recent_time, limit=50
        ):
        ...
        export_data = []
        async for dp in sql_writer.query_data_points(symbol_codes=["0108"], limit=10):
            export_data.append(
                {
        ...
        async for frame in sql_writer.query_frames(limit=3):
            frame_count += 1
            print(
                f"  Frame {frame_count}: {len(frame.data_points)} parameters from {frame.source}"
            )
    ...
    print(
        f"  witskit sql-query sqlite:///{db_path} --symbols '0108' --format csv --output depths.csv"
    )
...
async def demo_file_integration():
    ...
    sample_files = ["examples/sample.wits", "examples/sample_comprehensive.wits"]
    ...
    try:
        ...
        for file_path in available_files:
            ...
            for frame_data in file_reader.stream():
                try:
                    frame = decode_frame(
                        frame_data, source=f"file://{file_path}", use_metric_units=False
                    )
                    batch.append(frame)
                    frames_processed += 1
