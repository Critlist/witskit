...
class PasonSerialReader(BaseTransport):
    ...
    def __init__(self, port: str, baudrate: int = 9600,
        ...
        """
        if not HAS_SERIAL:
            raise ImportError("pyserial package is required for serial communication. Install with: pip install pyserial")
            ...
            while True:
                try:
                    if self.serial_conn.in_waiting > 0:
                        chunk: bytes = self.serial_conn.read(self.serial_conn.in_waiting)
                        if chunk:
