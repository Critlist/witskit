...
class TestUnitConverter:
    ...
    def test_pressure_conversions(self) -> None:
        ...
        result: float = UnitConverter.convert_value(1000.0, WITSUnits.KPA, WITSUnits.PSI)
    ...
    def test_mud_density_conversions(self) -> None:
        ...
        result: float = UnitConverter.convert_value(1200.0, WITSUnits.KGM3, WITSUnits.PPG)
    ...
    def test_temperature_conversions(self) -> None:
        ...
        result: float = UnitConverter.convert_value(150.0, WITSUnits.DEGF, WITSUnits.DEGC)
    ...
    def test_length_conversions(self) -> None:
        ...
        result: float = UnitConverter.convert_value(1000.0, WITSUnits.METERS, WITSUnits.FEET)
    ...
    def test_get_conversion_factor(self) -> None:
        ...
        factor: float | None = UnitConverter.get_conversion_factor(WITSUnits.KPA, WITSUnits.PSI)
...
class TestDrillingScenarios:
    ...
    def test_offshore_drilling_scenario(self) -> None:
        ...
        water_depth_ft: float = UnitConverter.convert_value(1500.0, WITSUnits.METERS, WITSUnits.FEET)
        ...
        pressure_kpa: float = UnitConverter.convert_value(2500.0, WITSUnits.PSI, WITSUnits.KPA)
        assert abs(pressure_kpa - 17236.9) < 1.0
        
        # Mud flow rate: 900 GPM = 3407 LPM
        flow_lpm: float = UnitConverter.convert_value(900.0, WITSUnits.GPM, WITSUnits.LPM)
        assert abs(flow_lpm - 3406.86) < 1.0
        
        # Mud density: 12.5 PPG = 1497 KG/M3
        density_kgm3: float = UnitConverter.convert_value(12.5, WITSUnits.PPG, WITSUnits.KGM3)
    ...
    def test_onshore_drilling_scenario(self) -> None:
        ...
        td_meters: float = UnitConverter.convert_value(8000.0, WITSUnits.FEET, WITSUnits.METERS)
        assert abs(td_meters - 2438.4) < 0.1
        
        # Drilling rate: 120 F/HR = 36.58 M/HR
        rop_metric: float = UnitConverter.convert_value(120.0, WITSUnits.FHR, WITSUnits.MHR)
        assert abs(rop_metric - 36.576) < 0.1
        
        # Bottom hole temperature: 180°F = 82.22°C
        bht_celsius: float = UnitConverter.convert_value(180.0, WITSUnits.DEGF, WITSUnits.DEGC)
        ...
        torque_knm: float = UnitConverter.convert_value(25.0, WITSUnits.KFLB, WITSUnits.KNM)
    ...
    def test_high_pressure_scenario(self) -> None:
        ...
        hp_kpa: float = UnitConverter.convert_value(15000.0, WITSUnits.PSI, WITSUnits.KPA)
        assert abs(hp_kpa - 103421.4) < 10.0
        
        # Heavy mud: 18 PPG = 2157 KG/M3
        heavy_mud: float = UnitConverter.convert_value(18.0, WITSUnits.PPG, WITSUnits.KGM3)
